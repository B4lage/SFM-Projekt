...
public class DataInController implements Initializable {
    ...
    private void handleMentesButtonClicked() throws IOException {
        ...
        {
            Alert alert = new Alert(Alert.AlertType.ERROR);
            alert.setTitle("Nem megfelelő súly!");
            alert.setHeaderText("Adj meg egy érvényes súly értéket kg-ban!");
            alert.showAndWait();
        }
        else if(!isValidMagassag)
        {
            Alert alert = new Alert(Alert.AlertType.ERROR);
            alert.setTitle("Nem megfelelő magasság!");
            alert.setHeaderText("Adj meg egy egész számot, ami megadja a magasságod cm-ben!");
            alert.showAndWait();
        }
        else if(nevErtek.getText() == "")
        {
            Alert alert = new Alert(Alert.AlertType.ERROR);
            alert.setTitle("Érvénytelen név!");
            alert.setHeaderText("Add meg a neved!");
            alert.showAndWait();
        }
        else
        {
